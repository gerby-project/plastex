...
def run(filename: str, config: ConfigManager):
    ...
    # gerby: offer to clean a non-empty output directory before the
    # (potentially very long) parse; only when interactive, so scripted
    # runs (CI, pytest) never block on input()
    outdir = config['files']['directory']
    if outdir and sys.stdin.isatty():
        jobname = os.path.basename(os.path.splitext(filename)[0])
